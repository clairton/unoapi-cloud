...
export class ClientForward implements Client {
  ...
  public async send(payload: any, options: any) {
    ...
    if (isUnoId(payload?.message_id)) {
      ...
      return { ok: { success: true }, error: undefined }
    } else if (payload?.status && !payload?.message_id) {
      logger.debug('Ignore status message %s without message_id', payload.status)
      return { ok: { success: true }, error: undefined }
    }
  ...
  public async assertSessions(_jids: string[], _force: boolean) {
    // sintax erro on vscode
    if (1 == 1) {
      throw 'ClientForward not assertSessions'
    }
    return false
  }
}
