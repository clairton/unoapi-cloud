...
export class ReloadBaileys extends Reload {
  ...
  async run(phone: string) {
    ...
    if (currentStatus == 'reloading') {
      logger.info('Ignore reload, already reloading session %s!', phone)
      return
    }
    if (currentStatus == 'restart_required') {
      logger.info('Ignore reload, restart required session %s!', phone)
      return
    }
