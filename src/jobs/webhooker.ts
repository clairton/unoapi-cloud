...
import { UNOAPI_DELAY_AFTER_FIRST_MESSAGE_WEBHOOK_MS, UNOAPI_EXCHANGE_BROKER_NAME, UNOAPI_QUEUE_WEBHOOKER } from '../defaults'
    ...
    if (a.webhooks) {
      ...
      await Promise.all(
        webhooks.map(async (webhook) => {
          return amqpPublish(UNOAPI_EXCHANGE_BROKER_NAME,  UNOAPI_QUEUE_WEBHOOKER, phone, { payload, webhook })
        }),
      )
    } else if (a.webhook) {
